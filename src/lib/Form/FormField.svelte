...
<label class="label {classValue}">
    ...
    <div class="flex rounded-md shadow-sm ring-1 ring-inset ring-gray-400 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md">
        ...
        {/if}
        <input required type="text" bind:value={value} name={id} id={id} autocomplete={id} class="block flex-1 border-0 bg-transparent py-1.5 pl-3 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6" placeholder={name}>
      </div>
</label>